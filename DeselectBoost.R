DeselectBoost <- function(object, data, fam, tau = NULL, method = c('attributable','cumulative')){
  require('plyr')
  tau = ifelse(is.null(tau), 0.01, tau)
  
  if(any(class(object) %in% 'mboost')){
    DeselectBoost_1(object, tau = tau,fam = fam, method = method[1])
  }else{
...
DeselectBoost_1 <- function(object, tau = NULL, fam, method = c('attributable','cumulative')){ # cumulative = FALSE
  require('plyr')
  ...
  Risk.order <- data.frame(Var,nameVar[count(select)[[1]]], as.numeric(Risk.Var))
  ...
  perc <- ifelse(is.null(tau), 0.01, tau) # Wenn kein Prozent wert angegeben wird, wird automatisch 0.01 genommen
  ...
  return(list(model = model_after,tau = perc, method = method[1]))
}
DeselectBoostLSS_2 <- function(object, data, fam, tau = NULL, method = c('attributable','cumulative')){
  require('plyr')
  data = data # Problem, wenn ich das nicht so übergeben, da die Struktur sich verändert?
  mstop <- mstop(object)[1] # wie ist das bei dem zyklischen?
  
  if(any(class(object) %in% "betaboost")){ # Unterscheidung wegen dem Intercept
    #nameVar <- names(object$coef(which=''))
    # form.type
    ...
    n.parameter <- c(names(object[[1]]$coef()),names(object[[2]]$coef())) # damit hätte ich die namen von mu/sigma ohne den intercept
    ...
    if(any(RiskRedOver$parameter == parameter[1]) && any(RiskRedOver$parameter == parameter[2])){  # Wenn beide Variablen enthalten
      form1 <- as.formula(paste(name.response, " ~ ", paste(RiskRedOver$VarName[RiskRedOver$parameter == parameter[1]], collapse= "+"))) # y heißt nicht immer y
      form2 <- as.formula(paste(name.response, " ~ ", paste(RiskRedOver$VarName[RiskRedOver$parameter == parameter[2]], collapse= "+"))) # y heißt nicht immer y
    }else{
      if(any(RiskRedOver$parameter == parameter[1])){ # Wenn das falsch, dann muss für mu nur der Intercept genommen werden
    ...
    if (length(risk(object, merge = T)) > (mstop + 2)) stop("risk cannot contain more entries than mstop")
    ...
    if(any(class(object) %in% "glmboostLSS")){
      model_after = glmboostLSS(formula, data = data, families = fam,  method = 'noncyclic', weights = model.weights(object), control = boost_control(mstop = mstop, nu = list(as.numeric(object[[1]]$control$nu),as.numeric(object[[2]]$control$nu))))
    }else{
      model_after = gamboost(formula, form2, data = data, families = fam, method = 'noncyclic', control = boost_control(mstop = mstop, nu = c(object[[1]]$control$nu,object[[2]]$control$nu)))
    }
  }
  
  return(list(model = model_after,tau = perc, method = method[1]))
}
DeselectBoostLSS_3 <- function(object, data, fam, tau = NULL, method = c('attributable','cumulative')){
  data = data # Problem, wenn ich das nicht so übergeben, da die Struktur sich verändert?
  mstop <- mstop(object)[1] # wie ist das bei dem zyklischen?
  ...
  which.response <- which(sapply(1:dim(data)[2], function(x){identical(as.numeric(data[,x]), as.numeric(object$mu$response))}))
  name.response <- colnames(data)[which.response]
  # select <- selected(LSS, merge = T)-1 # - Intercept
  # select <- select[select != 0] # hm eine liste, wo 2 einträge sind?
  select <- selected(object, parameter = names(object))
  select1 <- selected(object, parameter = names(object))# damit ich diesen vetor verändern kann
  select[[1]] <- select[[1]]-1  # für mu
  select[[2]] <- select[[2]]-1  # für sigma
  select[[3]] <- select[[3]]-1
  select[[1]] <- select[[1]][select[[1]] !=0]
  select[[2]] <- select[[2]][select[[2]] !=0]
  ...
  n.parameter <- c(names(object[[1]]$coef()),names(object[[2]]$coef()),names(object[[3]]$coef()))# damit hätte ich die namen von mu/sigma ohne den intercept
  if('(Intercept)' %in% n.parameter)  n.parameter<-n.parameter[-which(n.parameter == '(Intercept)')]
  ...
  if(any(RiskRedOver$parameter == parameter[3])){
    form3 <- as.formula(paste(name.response, " ~ ", paste(RiskRedOver$VarName[RiskRedOver$parameter == parameter[3]], collapse= "+"))) # y heißt nicht immer y
  ...
  if(any(class(object) %in% "glmboostLSS")){
    ...
    model_after = glmboostLSS(formula, data = data, families = fam, method = 'noncyclic', weights = model.weights(object), control = boost_control(mstop = mstop, nu = list(as.numeric(object[[1]]$control$nu),as.numeric(object[[2]]$control$nu),as.numeric(object[[3]]$control$nu))))
    
  }else{
    model_after = gamboostLSS(formula, data = data, families = fam, method = 'noncyclic', weights = model.weights(object), control = boost_control(mstop = mstop, nu = list(as.numeric(object[[1]]$control$nu),as.numeric(object[[2]]$control$nu),as.numeric(object[[3]]$control$nu))))
  }
  
  return(list(model = model_after,tau = perc, method = method[1]))
}
DeselectBoostLSS_4 <- function(object, data, fam, tau = NULL, method = c('attributable','cumulative')){
  data = data # Problem, wenn ich das nicht so übergeben, da die Struktur sich verändert?
  mstop <- mstop(object)[1] # wie ist das bei dem zyklischen?
  ...
  n.parameter <- c(names(object[[1]]$coef()),names(object[[2]]$coef()),names(object[[3]]$coef()),names(object[[3]]$coef()))# damit hätte ich die namen von mu/sigma ohne den intercept
  if('(Intercept)' %in% n.parameter)  n.parameter<-n.parameter[-which(n.parameter == '(Intercept)')]
  ...
  if(any(RiskRedOver$parameter == parameter[4])){
    form4 <- as.formula(paste(name.response, " ~ ", paste(RiskRedOver$VarName[RiskRedOver$parameter == parameter[4]], collapse= "+"))) # y heißt nicht immer y
  ...
  if(any(class(object) %in% "glmboostLSS")){
    ...
    model_after = glmboostLSS(formula, data = data, families = fam, method = 'noncyclic', weights = model.weights(object), control = boost_control(mstop = mstop, nu = list(as.numeric(object[[1]]$control$nu),as.numeric(object[[2]]$control$nu),as.numeric(object[[3]]$control$nu),as.numeric(object[[4]]$control$nu))))
    
  }else{
    model_after = gamboostLSS(formula, data = data, families = fam, method = 'noncyclic', weights = model.weights(object), control = boost_control(mstop = mstop, nu = list(as.numeric(object[[1]]$control$nu),as.numeric(object[[2]]$control$nu),as.numeric(object[[3]]$control$nu),as.numeric(object[[4]]$control$nu))))
  }
