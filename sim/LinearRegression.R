...
fsim <- function(seed, n, p, pinf, n.test, mu, sigma.y, corr, tau){
  ...
  beta = c(-2,-1.5,-1,1,1.5,2,rep(0,p-6))
  mean_X = rep(mu,length = p)
  sigma_X = toeplitz(sapply(seq(0,p-1), function(x) corr^x))
  
  set.seed(seed) 
  X = rmvnorm(n, mean = mean_X, sigma = sigma_X)
  dat.train = data.frame(X, y = rnorm(n, X %*% beta, sigma.y))
  
  X.test = rmvnorm(n.test ,mean = mean_X, sigma = sigma_X)
  dat.test = data.frame(X.test, y = rnorm(n.test, X.test %*% beta, sigma.y))
  
  glm1 <- glmboost(y ~., data = dat.train) #, control = boost_control(mstop = 10000))  # ,trace = T
  cv25 <- cv(model.weights(glm1),type = 'kfold')
  cvr <- cvrisk(glm1, folds = cv25, grid = 1:3000, papply = mclapply)
  stopIT <- mstop(cvr)
  glm1[mstop(cvr)]
...
save(results, file = "DeselectionLinearRegression.RData")
