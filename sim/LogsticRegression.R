...
fsim <- function(seed, n, p, pinf, n.test, mu, sigma.y, corr, tau){
  ...
  for(k in tau){
    ...
    AUC[[i]] <- roc(dat.test$y, as.numeric(predict(glm1_after, newdata = dat.test,type = "response")))$auc
    Brier[[i]] <- mean((as.numeric(predict(glm1_after, newdata = dat.test,type = "response"))-(as.numeric(dat.test$y)-1))^2)
  }
